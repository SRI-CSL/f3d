...
#|
FOREIGN-POINTER-ADDRESS and SB-ALIEN:UNSIGNED-LONG is used for passing addresses of arrays instead of 
SB-ALIEN::VECTOR-SAP in o0rder to eliminate consing on SAPs.  
|#
(defparameter *foreign-address-type* 'sb-alien:unsigned-long)
...
(defparameter *foreign-array-address-type* 'sb-alien:unsigned-long)

#| PROBLEM:  Tue Feb 24 2009
    ..../src/code/target-c-call.lisp contains:
    (define-alien-type unsigned-long (unsigned #.sb!vm::n-machine-word-bits))
    (define-alien-type unsigned-long-long (unsigned 64))

   The problem is that unsigned-long on X86-64 is 64, not 32.  
   This might break things in qffi.
   Perhaps all uses of unsigned-long are for addresses, in which case a synonym should be used
   in all declarations for address slots.
...
(defun qffi-to-sbcl-type (type &optional return-type)
  (case type 
    ...
    ((:simple-string :string ) 'sb-alien:c-string)
    (otherwise
     (cond ;;((keywordp type) type)
       ((symbolp type)
	(let ((expansion (expand-foreign-type type)))
	  (if (not (equal type expansion))
	      (qffi-to-sbcl-type expansion)
	      (error "QFFI-TO-SBCL-TYPE Unknown qffi foreign type -> ~S" type))))
       ((consp type)
	(case (car type)
	  (:pointer 
...
(defpackage :config) ; needed for system configuration options
...
;;;
;;; Error occurs in simple-string declarations within SBCL
(defvar config::def-foreign-function-arg-checking-default t)

(defvar config::def-foreign-function-arg-checking-decls
  #+sbcl nil
  #+cmu '(ext:optimize-interface (safety 1) (speed 3))
  )


(defun sbcl-expand-def-foreign-function (parser-fn args)
  ...
  (multiple-value-bind (kw-opts lisp-fn-name c-name qffi-return-type arglist arg-decls vars 
	...
	(multiple-value-bind (sbcl-return-type retform) (qffi-to-sbcl-type qffi-return-type t)
	  (loop with convert-fns-exist
		for var in vars
		for qffi-type in qffi-c-types
		with (sbcl-c-type convert-fn)
		do (multiple-value-setq (sbcl-c-type convert-fn) (qffi-to-sbcl-type qffi-type))
		collect sbcl-c-type into sbcl-c-types
		...
		collect (list var sbcl-c-type) into c-arg-names-types
		finally
	     (return  
		   ...
		   `(progn
		      ...
		      (sb-alien:define-alien-routine (,c-name ,lisp-fn-name) ,sbcl-return-type
			. ,c-arg-names-types))
		   (let ((ff-name (intern (format nil "~a%internal" lisp-fn-name) (symbol-package lisp-fn-name))))
		     `(progn
			(declaim (inline ,ff-name))
			#+cmu  (declaim (optimize (extensions:inhibit-warnings 3)))
			(sb-alien:define-alien-routine (,c-name ,ff-name) ,sbcl-return-type
			  . ,c-arg-names-types)
			...
			(defun ,lisp-fn-name ,arglist
			  ...
			  ,(let ((body `(,ff-name . ,argforms)))
				...
				(when without-gcing
				  (setq body `(sb-sys::without-gcing  ,body)))
				(when without-interrupts
				  (setq body `(sb-sys::without-interrupts  ,body)))
				body))
			',lisp-fn-name))))))))))
...
(defun find-alien-callback-sap-for-function-name (name)
  (loop for callback-sap being the hash-values of sb-alien::*alien-callbacks*
	using (hash-key key)
;; Odd discrepancy - the commented line is the old version, but the
;; hash key that appears in the table seems to follow the uncommented
;; convention:
	for #+old (specifier . function) #-old (specifier function) = key
	when (eq (symbol-function name) function)
	  return callback-sap))

;(sb-alien::sap-int (find-alien-callback-for-function-name 'cl-user::double*-cmp))
...
(defun register-foreign-callback (callback-fn trampoline-address-symbol)
  ...
  (setf (foreign-variable-value trampoline-address-symbol 'sb-alien:unsigned-long) ; FIXME
	(sb-alien::sap-int (find-alien-callback-sap-for-function-name callback-fn)))) ; FIXME
...
(defun qffi-to-sbcl-callback-return-type (qffi-type)
  (if (null qffi-type) 
      'sb-alien::unsigned-int ;; This is not quite right, but void doesn't work on the 11/2004 snapshot of sbcl
;;      'sb-alien::void
      (qffi-to-sbcl-type qffi-type t)))



(defun sbcl-expand-def-foreign-callable (parser-fn args)
  (multiple-value-bind (name c-name ret-type c-args-and-qffi-types body)
      (funcall parser-fn args)
    (let ((c-args-and-sbcl-types 
	   (loop for (var qffi-type) in c-args-and-qffi-types
		 collect `(,var ,(qffi-to-sbcl-type qffi-type t)))))
      `(progn (sb-alien::define-alien-callback  ; Helmut Eller's callback machinery
		  ,name 
		  ,(qffi-to-sbcl-callback-return-type ret-type)
		  ,c-args-and-sbcl-types
		.,body)
	      ,@(when c-name
		      `((register-foreign-callback ',name ,(format nil "~a_callback_address" c-name))))))))

;;; Fix a problem when an arg requires a conversion function.
(defun sbcl-expand-def-foreign-callable (parser-fn args)
  (multiple-value-bind (name c-name ret-type c-args-and-qffi-types body)
      (funcall parser-fn args)
    (let ((c-args-and-sbcl-types 
	   (loop for (var qffi-type) in c-args-and-qffi-types
		 collect `(,var ,(qffi-to-sbcl-type qffi-type t))))
	  (arg-conversions (loop for (var qffi-type) in c-args-and-qffi-types
				 for convert-fn = (nth-value 1 (qffi-to-sbcl-type qffi-type t))
				 when convert-fn
				   collect `(,var @(,convert-fn ,var)))))
      `(progn (sb-alien::define-alien-callback  ; Helmut Eller's callback machinery
		  ,name 
		  ,(qffi-to-sbcl-callback-return-type ret-type)
		  ,c-args-and-sbcl-types
		(let ,arg-conversions .,body))
	      ,@(when c-name
		      `((register-foreign-callback ',name ,(format nil "~a_callback_address" c-name))))))))
...
(defun build-slot-accessors (struct-name slot-name)
  (let* ((pkg (symbol-package struct-name))
	...
	;;(declare (optimize (speed 3) (safety 0)))
	(declare (type (sb-alien:alien (* ,struct-name)) obj))
	(sb-alien:slot obj ',slot-name))
      (defun (setf ,getter) (val obj)
	;;(declare (ext:optimize-interface (speed 3) (safety 0)))
	;;(declare (optimize (speed 3) (safety 0)))
	(declare (type (sb-alien:alien (* ,struct-name)) obj))
	(setf (sb-alien:slot obj ',slot-name) val)))))
...
(defun build-slot-accessors (struct-name slot-name slot-type)
  (let* ((pkg (symbol-package struct-name))
	...
	(foreign-type-struct-slot-array-index-info slot-name slot-type pkg)
      (let* ((ref-form0 `(sb-alien:slot obj ',slot-name))
	     (ref-form (if indices `(sb-alien::deref ,ref-form0 .,indices) ref-form0)))
	`(locally
	  ;(declare (ext:optimize-interface (speed 3) (safety 0)))
	  ;(declare (optimize (speed 3) (safety 0)))
	  #+cmu(declare (optimize (extensions:inhibit-warnings 3)))
...
(defun qffi-to-sbcl-struct-slot-type (qffi-type)
  (if (consp qffi-type)
      (case (car qffi-type)
	((:array :simple-array :simple-array-or-null :array-or-null)
	 `(array ,(qffi-to-sbcl-type (cadr qffi-type)) .,(caddr qffi-type))
	 )
	(otherwise (qffi-to-sbcl-type qffi-type)))

      (qffi-to-sbcl-type qffi-type)))
	

;;; Restrict to the form (DEF-FOREIGN-STRUCT name (slotdef ...))

(defun sbcl-expand-def-foreign-struct (parser-fn args)
  (multiple-value-bind (name slot-names-and-qffi-types) (funcall parser-fn args )
    (let ((creator `(defun ,(intern (format nil "MAKE-~a" name) (symbol-package name)) ()
		     (sb-alien:make-alien (sb-alien:struct ,name)))))
      (loop for (slot-name qffi-type) in slot-names-and-qffi-types
	    collect `(,slot-name ,(qffi-to-sbcl-struct-slot-type qffi-type)) 
	      into slot-names-and-sbcl-types
	    collect (build-slot-accessors name slot-name qffi-type) into accessors
	    finally 
	 (return `(progn 
		   (eval-when (compile load eval)
		   (sb-alien::define-alien-type ,name (sb-alien:struct ,name .,slot-names-and-sbcl-types)))
		   ,creator
		   ,@accessors
...
(defmacro def-foreign-function (&rest args)
  (sbcl-expand-def-foreign-function #'parse-lcl-def-foreign-function args))

(defmacro def-foreign-callable (&rest args)
  (sbcl-expand-def-foreign-callable #'parse-lcl-def-foreign-callable args))

(defmacro def-foreign-struct (&rest args)
  (sbcl-expand-def-foreign-struct #'parse-lcl-def-foreign-struct args))
...
(defmacro def-foreign-function (&rest args)
  (let ((expansion (sbcl-expand-def-foreign-function #'parse-lcl-def-foreign-function args)))
...
(defmacro def-foreign-callable (&rest args)
  (let ((expansion (sbcl-expand-def-foreign-callable #'parse-lcl-def-foreign-callable args)))
