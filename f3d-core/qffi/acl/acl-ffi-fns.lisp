...
#+never ; already defined in $FREEDIUS/lisp/lcl/acl-lcl-fns.lisp
(defun %POINTER (x)
...
(ff:def-c-type callback-char-place :char)
...
(defun foreign-variable-value (name)
...
(defun (setf foreign-variable-value) (value name)
  (let ((ptr (foreign-variable-pointer name )))
    (cond ((null ptr)
	   (format t ";;; foreign-variable-value warning: symbol ~a is undefined~%"
		   name))
