...
#else
  // Something odd is happening here - the above definition results in
  // arrays whose "length" is 4x the allocated memory, so this is a
  // hack borne of ignorance:
#define MAKE_FIXNUM(x)((x)<<(N_LOWTAG_BITS-3)) // x86-64 lowtag is 4 bits, but there are odd and even fixnums
#endif
...
#define EXTRA_PREAMBLE 0 
  ...
  //  fprintf(stderr,"HAVE_POSIX_MEMALIGN is false, alloc_nbytes=%d\n", alloc_nbytes);
...
#else  
  //  fprintf(stderr,"HAVE_POSIX_MEMALIGN is true, alloc_nbytes=%d\n", alloc_nbytes);
  ...
  *(vect_hdr_ptr-1) = malloc_offset; // save the malloc_offset immediately before the lisp header.
  ...
  //  fprintf(stderr,"length=%d, vect->length=%d\n", length, vect->length);
