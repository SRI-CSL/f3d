...
set default_scrollbar_width 15
...
proc qframe {w args} {
    eval {labelled_widget frame $w} $args
}

proc qlabel {w args} {
    eval {labelled_widget label $w} $args
}
...
proc qentry {w args} {
    ...
    set initial_value_p [arefp_remove arg -initial_value val]
    set w [eval {labelled_widget entry $w} [array get arg]]
    if {$formatp} {    
...
proc qbutton {w args} {
    eval {labelled_button button $w} $args
}
proc qflatbutton {w args} {
    eval {labelled_button button $w -borderwidth 0 -highlightthickness 0}  $args
}
 

proc qcheckbutton {w args} {
    eval {labelled_button checkbutton $w} $args
}
...
proc qtogglebutton {w args} {
    ...
    set w [eval {labelled_button checkbutton $w -indicatoron false -variable $varName} [array get arg]]
...
proc qtogglebutton {w args} {
    set w [eval {labelled_button checkbutton $w -indicatoron false} $args]
...
proc qradiobutton {w args} {
    ...
    eval {labelled_button radiobutton $w -value $value} [array get arg] 
}
...
proc qoptionbutton {w args} {
    eval {labelled_widget menubutton $w -qwidget_class qoptionbutton} $args
}
...
proc qcme_keypress_handler {w action keysym char} {
    puts "Key pressed. $w $keysym $char"
...
# Not needed anymore.  
# CREATE-TKGLWIN-BINDINGS in tk-window-panel.lisp creates the event bindings to Lisp handlers.
